...
Use `benchmark/AGENT_PROMPT_BLIND.md` and `benchmark/BLIND_CHALLENGE_PROTOCOL.md` for blind B-mode agent runs. For a clean challenge, give the model only the blind pack generated by `benchmark/tools/make_blind_pack.sh`. Keep this full repository as the reference solution and baseline record.
